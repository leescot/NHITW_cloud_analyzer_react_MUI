...
18. **藥品處方整合 ATC 碼 (2026-01-01)**
    - 修改 `medicationProcessor.js` 確保 `atc_code` 欄位被正確提取。
    - 更新 `gaiCopyFormatter.js` 中的 `formatMedication` 邏輯，將 ATC 碼包含在產出的 XML/純文字中。
    - 效益：AI 現在具備藥理分類資訊，能提供更深度的用藥分析。

19. **GAI 預設模板調整與新增 ATC 分類 (2026-01-01)**
    - 移除「危險警示」模板。
    - 重構「藥物交互作用」模板，聚焦於藥物間的相互作用。
    - 優化「檢驗異常值」模板，要求 TL;DR 精簡輸出。
    - **新增「藥品 ATC 分類」**：依 WHO ATC 2025 規則進行分類，並優化 prompt 以支援 Table 格式排列藥物群組及其日期。

20. **配置介面過濾器修正 (2026-01-01)**
    - 修正 `SidebarV2ConfigDialog.jsx` 中的過濾邏輯，使其包含 `advanced` 類別，確保「藥品 ATC 分類」可被選擇。

21. **推理模型 (Reasoning Models) 穩定性提升 (2026-01-01)**
    - **Token 限制提升**：將 `CerebrasProvider` 與 `GroqProvider` 的限制提高至 16,384，解決推理過程過長導致截斷的問題。
    - **推理內容提取**：實作 `reasoning` 回傳欄位自動備援機制。
    - **UI 優化**：新增截斷警告標記與 `<think>` 標籤過濾清理。

22. **GAI Sidebar UI 全面實作優化與版面刷新 (2026-01-01)**
    - **Tab 1 (自動分析) 優化**:
        - 移除結果區域的嵌套 Paper 外框，使內容直接貼合側邊欄，最大化顯示空間。
        - 縮減底部統計資訊 (tokens/time) 的視覺佔比，提升資訊密度。
    - **Tab 2 (快速按鈕) 佈局與邏輯重構**:
        - 實作「上方按鈕網格、下方共享結果」的現代化佈局。
        - **取消起始預設選中**: 頁面載入時不自動高亮任何按鈕，消除「已分析完畢」的誤會。
        - **執行邏輯修復**: 點擊按鈕即立即觸發該槽位的分析請求，不再受限於選中狀態。
        - **視覺更新**: 選中按鈕改用 primary 邊框與淺藍背景 (Outlined)，取代深色 Fill。
    - **Tab 3 (Chat 對話) 空間極大化設計**:
        - **寬度刷新**: 移除 bulky 的圓形頭像與水平位移，改用 100% 全寬對話氣泡。
        - **標籤系統**: User/AI 標籤放置於氣泡左上方，節省水平寬度。
        - **快速提問換列**: Chips 改為 `flex-wrap` 佈局，解決小寬度下看不到後續問題的問題。
    - **配置對話框 (SidebarV2ConfigDialog) 現代化**:
