...
1. [x] 檢驗 tab「複製全部」→ 有 Snackbar 提示、剪貼簿正確(bug 修復驗證)
2. [x] 西藥自訂複製格式編輯器完整操作:拖曳(視覺回饋)、增刪元素、ICD 元素加入、預覽、儲存、重置;**確認之前儲存的自訂格式載入不變**
3. [x] 檢驗自訂複製格式編輯器完整操作:同上 +「加入基本日期組合」按鈕
4. [x] 西藥列表複製(單一 visit + 複製全部)以自訂格式輸出正確(側通道移除驗證——這條最重要)
5. [x] 本地匯入後再複製,自訂格式仍生效
...
- [x] 手動煙霧測試(編輯器操作、複製功能實測)通過(2026-07-03 維護者實測)
