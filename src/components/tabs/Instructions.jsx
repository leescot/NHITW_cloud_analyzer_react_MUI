...
1. 點選 Chrome 右上角的「擴充功能」圖示 (拼圖形狀)
2. 找到「健保雲端藥歷整理器2.0」，點選旁邊的「釘選」圖示

IMAGE_PLACEHOLDER(pin_extension_01, 點選旁邊的「釘選」圖示)
...
- 點選「雲」圖示，就可開啟設定面板。設定功能，請參照其它說明頁文件
...
### 2. 點選網頁右方的「雲」圖示，可開啟以上資訊整合界面

IMAGE_PLACEHOLDER(icon128, 點選「雲」圖示)


- 點選圖示出視的整合資訊畫面(此為健保雲端的測試病人資料)
...
總覽顯示設定頁面讓您能夠自訂「總覽」頁面上顯示的資料範圍及內容，包括藥物、檢驗和影像追蹤的時間範圍，以及您關注的特定檢驗項目和影像檢查型別。
...
點選「關注檢驗清單設定」按鈕可開啟設定對話框，您可以：
- 勾選或取消勾選要在總覽頁面顯示的檢驗項目
- 使用上下箭頭按鈕調整檢驗項目的顯示順序
- 點選「重設為預設」恢復系統建議的預設檢驗項目及排序
- 點選「保存」儲存您的自訂設定，或「取消」放棄變更
...
點選「關注影像清單設定」按鈕可開啟設定對話框，您可以：
- 勾選或取消勾選要在總覽頁面顯示的影像檢查項目
- 點選「重設為預設」恢復系統建議的預設影像檢查項目
- 點選「保存」儲存您的自訂設定，或「取消」放棄變更
...
西藥顯示設定頁面讓您可以自訂擴充功能對於藥物資訊的顯示方式、複製格式，以及設定藥物型別的顏色分類功能。
...
ATC5 變色功能可讓您依照藥物分類為不同藥物設定顏色標示，幫助您快速識別不同型別的藥物。
...
## 顯示藥物外觀功能(需連接外網)
**注意：此功能需連接外網**
將在藥物的右側出現圖片的icon，點選即可顯示藥物的外觀圖片。
...
- **依檢驗型別分組**：按照檢驗型別（如：血液學、生化、電解質等）分類顯示，預設選項
