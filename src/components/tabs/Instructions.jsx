...
IMAGE_PLACEHOLDER(open_setting_page, 開啟設定頁面)
   `,
  },
  {
    id: 'features',
...
- **總覽**：整合使用者想關注的「西藥」+「檢驗」+「影像」，與「訊息+手術+過敏+住院」的整合畫面

- **西藥**：西藥清單，直式排列，時間越近在最上面

- **西藥(表格)**：整合式西藥整理表格

- **中藥**：中藥清單，直式排列，時間越近在最上面

- **檢驗**：檢驗報告清單，直式排列，時間越近在最上面
...
IMAGE_PLACEHOLDER(setting_general, 一般顯示設定頁面內容)
...
IMAGE_PLACEHOLDER(setting_medication, 西藥顯示設定頁面內容)

我會將 ATC5 預設值整合到西藥顯示設定的使用者指南中。
markdownCopy# 西藥顯示設定 - 使用者指南
...
- 針對腎臟科需求，可注意 NSAID, ACEI, ARB 等可能影響腎功能的藥物
- 針對心臟科需求，可重點關注 ACEI, ARB, STATIN 等常用心臟用藥
- 針對內分泌科需求，可突顯 SGLT2, GLP1 等降血糖藥物
...
IMAGE_PLACEHOLDER(setting_chinesemedication, 中藥顯示設定頁面內容)
...
- **關閉複製功能**：停用中藥資訊的複製功能
- **複製商品名(直式)**：僅複製中藥名稱，每種中藥一行
- **複製商品名+使用量(直式)**：複製中藥名稱及劑量資訊，每種中藥一行，預設選項
- **複製商品名(橫式)**：僅複製中藥名稱，所有中藥在同一行，以逗號分隔
- **複製商品名+使用量(橫式)**：複製中藥名稱及劑量資訊，所有中藥在同一行

## 使用建議

- 若您需要仔細分析中藥處方與患者病況的關聯，建議開啟「顯示主診斷資訊」
- 中醫師或中藥專業人員可能會希望開啟「顯示效能名稱」，便於快速辨識處方的治療目的
- 複製格式的選擇取決於您的文件記錄需求：
  - 撰寫詳細病歷時，建議使用直式格式
  - 需要簡潔摘要時，可使用橫式格式
...
中藥顯示設定專為中藥處方設計，相較於西藥設定，它更加簡潔且針對中藥特性進行優化。中藥設定不包含 ATC5 變色功能，因為中藥分類系統與西藥不
`,
  },
  {
    id: 'setting_labdata',
...
IMAGE_PLACEHOLDER(setting_labdata, 檢驗顯示設定頁面內容)
...
- 根據您的螢幕大小和閱讀習慣選擇適合的呈現方式：
  - 小螢幕裝置可能適合使用「直式呈現」
  - 大螢幕時「雙欄」或「三欄」呈現可以更有效利用空間
  - 專業人士通常偏好「依檢驗類型分組」，因為這與臨床思考方式一致
- 複製格式的選擇應基於您的文檔需求：
  - 撰寫詳細病歷時，「直式格式」較為適合
  - 製作簡報或摘要時，「橫式格式」更為簡潔
  - 系統性分析時，「分類格式」能保持數據的邏輯關係
`,
  },
  {
    id: 'icon_status',
