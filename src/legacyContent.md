...
10. **慢性處方箋 (chronicMed) 支援**：
    - 端點：`/imu/api/imue0008/imue0008s05/get-data`，對應主選單節點 `2.3`
    - 已加入 `fetchAllDataTypes` 常規 `dataTypes` 陣列，每次抓取時主動 fetch
    - 該 API 回應與其他端點不同，**沒有 `rObject` 欄位**，而是回傳 `{ chrDataN: [...], chrDataY: [...] }`：
      - `chrDataN`：`chr_days` / `chr_num` 為 `null`、`overdue: "N"` 的紀錄（推測為「未啟用慢箋週期」的歷史紀錄）
      - `chrDataY`：`chr_days` / `chr_num` 有值、`overdue: "Y"` 或 `"N"` 的紀錄（推測為實際慢箋紀錄）
    ...
    - 目前僅做抓取/下載/載入；尚未撰寫 processor 與 UI 頁籤，等樣本累積後再規劃顯示邏輯
