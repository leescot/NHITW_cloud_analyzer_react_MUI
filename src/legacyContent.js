...
function isOnLoginPage() {
  ...
  return url.includes('medcloud2.nhi.gov.tw/imu/login') || 
...
function isOnTargetPage() {
  ...
  return url.includes('medcloud2.nhi.gov.tw/imu/IMUE1000/IMUE0008') || 
         ...
         url.includes('medcloud2.nhi.gov.tw/imu/IMUE2000/IMUE2000');    // Patient summary
}
      ...
      chrome.storage.local.get('currentUserSession', function(result) {
        ...
        if (isNewSession) {
          ...
          chrome.storage.local.set({ currentUserSession: userInfo }, () => {
            ...
            chrome.runtime.sendMessage({ action: 'userSessionChanged', userSession: userInfo });
            resolve(true); // 表示新病人
          });
        } else {
...
async function extractUserInfo() {
  ...
  // 方法3: 從 DOM 獲取（次要備用）
  try {
    const patientInfoElements = document.querySelectorAll('.patient-info, .patient-name, .card-no');
      ...
      if (isOnTargetPage()) {
        ...
        checkAndInitUserSession().then((isNewSession) => {
          if (isNewSession || !window.lastInterceptedMedicationData?.rObject) {
            console.log("New patient or no data due to URL change, fetching...");
            fetchAllDataTypes();
          } else {
        ...
        const newHandler = function() {
          ...
          if (originalUrl && (
            ...
            if (this.readyState === 4 && this.status === 200) {
              try {
                console.log(`Complete XHR response received, size: ${this.responseText.length} bytes`);
                ...
                if (data && (recordsArray && Array.isArray(recordsArray) || originalUrl.includes(MASTER_MENU_API_URL_PATTERN))) {
                  ...
                  // 判斷是藥歷還是檢驗資料
                  const dataType = originalUrl.includes(MEDICATION_API_URL_PATTERN) ? 'medication' :
                                   originalUrl.includes(LABDATA_API_URL_PATTERN) ? 'labdata' :
                                   originalUrl.includes(CHINESE_MED_API_URL_PATTERN) ? 'chinesemed' :
                                   originalUrl.includes(IMAGING_API_URL_PATTERN) ? 'imaging' :
                                   originalUrl.includes(ALLERGY_API_URL_PATTERN) ? 'allergy' :
                  ...
                  if (dataType) {
                    ...
                    console.log(`Successfully intercepted ${recordsArray.length} ${dataType} records via XHR`);
                    ...
                    console.log('Response does not contain expected data structure:', data);
                  }
                }
              } catch (error) {
    ...
    if (url && (
      ...
      try {
        ...
        if (response.ok) {
          ...
          clonedResponse.text()
            .then(text => {
              console.log(`Complete fetch response received, size: ${text.length} bytes`);
              ...
              try {
                ...
                if (data && (recordsArray && Array.isArray(recordsArray) || url.includes(MASTER_MENU_API_URL_PATTERN))) {
                  ...
                  // 判斷是藥歷還是檢驗資料
                  const dataType = url.includes(MEDICATION_API_URL_PATTERN) ? 'medication' :
                                   url.includes(LABDATA_API_URL_PATTERN) ? 'labdata' :
                                   url.includes(CHINESE_MED_API_URL_PATTERN) ? 'chinesemed' :
                                   url.includes(IMAGING_API_URL_PATTERN) ? 'imaging' :
                                   url.includes(ALLERGY_API_URL_PATTERN) ? 'allergy' :
                  ...
                  if (dataType) {
                    ...
                    console.log(`Successfully intercepted ${recordsArray.length} ${dataType} records via fetch`);
                    ...
                    console.warn('Fetch response does not contain expected data structure:', data);
                  }
                }
              } catch (error) {
    ...
    if (url && (
      ...
      this.addEventListener('loadend', function() {
        if (xhr.status === 200 && xhr._requestHeaders) {
          ...
          console.log('Headers captured:', Object.keys(lastSuccessfulRequestHeaders).join(', '));
...
function extractAuthorizationToken() {
  ...
  // 方法1: 優先使用已捕獲的請求頭中獲取
  if (lastSuccessfulRequestHeaders && lastSuccessfulRequestHeaders['Authorization']) {
  ...
  try {
    const possibleTokenNames = ['jwt_token', 'token', 'access_token', 'auth_token'];
  ...
  try {
    ...
    for (let script of scripts) {
      if (script.textContent && script.textContent.includes('token')) {
        const tokenMatch = script.textContent.match(/token\s*=\s*["']([^"']+)["']/);
  ...
  try {
    const storageToken = localStorage.getItem('nhi_extractor_token') || localStorage.getItem('jwt_token');
    if (storageToken) {
      // 檢查令牌是否過期或受污染
      if (validateToken(storageToken)) {
        console.log('Found valid token in localStorage');
        return storageToken.startsWith('Bearer ') ? storageToken : 'Bearer ' + storageToken;
      } else {
...
function fetchAllDataTypes() {
  ...
  const hasPendingRequests = Object.values(pendingRequests).some(pending => pending);
...
function injectFloatingIcon() {
  ...
  div.innerHTML = `
    <div style="position: fixed; right: 20px; top: 50%; transform: translateY(-50%); z-index: 9999;">
      <img src="${chrome.runtime.getURL('images/icon-128.png')}" style="width: 48px; height: 48px; cursor: pointer;" />
    </div>
  `;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "settingChanged") {
    ...
    const isMedicationSetting = ['simplifyMedicineName', 'showDiagnosis', 'showGenericName', 'enableATC5Coloring', 'copyFormat'].includes(message.setting);
    
    const isChineseMedSetting = ['chineseMedShowDiagnosis', 'chineseMedShowEffectName', 'chineseMedCopyFormat'].includes(message.setting);
    ...
    try {
      ...
      for (const [key, value] of Object.entries(patientData)) {
        if (value && typeof value === 'object') {
          if (value.rObject && Array.isArray(value.rObject) && value.rObject.length > 0) {
            hasAnyData = true;
            break;
          }
        }
      ...
      if (hasAnyData) {
        ...
        const date = new Date();
        const fileName = `patient_data_${date.getFullYear()}${(date.getMonth()+1).toString().padStart(2, '0')}${date.getDate().toString().padStart(2, '0')}_${date.getHours().toString().padStart(2, '0')}${date.getMinutes().toString().padStart(2, '0')}.json`;
